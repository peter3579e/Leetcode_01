...
object Regex {
  ...
  The developers at Amazon are developing a regex matching library for their NLP use cases. A prototype regex matching has the following requirements:
        ...
        • If an expression is enclosed in parentheses ('and')', it is treated as one expression and any asterisk
            * applies to the whole expression. It is guaranteed that no expression enclosed within parenthesis contains any '*' but is always followed by '**. Also, there is no nested brackets sequence in the given regex expression for the prototype.
  For example,
  ...
  Example
  Suppose, n = 8, regex = "(a.b)*bd", k = 3, arr= ["acbabbbd", "bd", "abbd"]• arrO] = "acbabbbd" matches the regex "(a.b)*bd", if we replace '*' with 2 occurrences of "a b".
i.e.
     */
}